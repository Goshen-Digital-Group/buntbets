...
const Wrapper = styled.div<{ $isMinimized: boolean }>`
  ...
  background: ${({ $isMinimized }) => ($isMinimized ? '#574a00ff' : 'rgba(44, 40, 2, 0.85)')};
  border: 1px solid
    ${({ $isMinimized }) => ($isMinimized ? 'rgba(255,255,255,0.3)' : 'rgba(251, 255, 0, 0.1)')};
  color: #eee;
  font-size: 0.9rem;
    ...
    <Wrapper $isMinimized={isMinimized}>
      ...
      <ContentContainer $isMinimized={isMinimized}>
        <Header onClick={toggleMinimize}>
          <HeaderTitle>Buntbets Chat</HeaderTitle>
          <HeaderStatus>
